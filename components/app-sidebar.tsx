...
const NAV_OPERATION: NavItemDef[] = [
  ...
  { key: "meus",        label: "Meus Produtos",  href: "/meus-produtos",       icon: Package,  badge: "12" },
  { key: "vitrine",     label: "Vitrine",         href: "/vitrine-de-produtos", icon: Store,    highlight: true },
  { key: "pedidos",     label: "Pedidos",         href: "/pedidos",             icon: Receipt,  badge: "4" },
];
            ...
            <div className="ml-[30px] mt-0.5 mb-1 flex flex-col gap-0.5 border-l border-[var(--ink-200)] pl-1">
              {item.children?.map((child) => {
                const isChildActive = activeKey === child.key ||
                  (typeof window !== "undefined" && window.location.pathname === child.href);
                return (
                  <Link
                    key={child.key}
                    href={child.href}
