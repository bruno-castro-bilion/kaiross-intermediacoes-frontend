...
export function useTokenExpiration() {
  ...
  const refreshToken = useCallback(async () => {
    ...
    try {
      ...
      // 401/403 = refresh inválido/expirado → logout. O BFF já limpou cookies.
      forceLogout();
      return false;
    } catch (error) {
      console.error("Erro ao renovar token:", error);
      // Erro de rede: estende localmente em vez de deslogar — o usuário
      // pode estar offline ou com instabilidade momentânea.
      localStorage.setItem(TOKEN_ISSUED_AT_KEY, Date.now().toString());
      setShowWarning(false);
      setTimeRemaining(WARNING_TIME_MS);
      if (countdownRef.current) {
